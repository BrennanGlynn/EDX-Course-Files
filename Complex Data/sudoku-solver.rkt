...
;; Board -> Board|false
;; Produce the solution to a given board or false if a solution doesn't exist
...
;; Board -> Boolean
;; Consumes a board and returns true if the board is full
...
;; Board -> (listOf Board)
;; Finds first empty position, fills it with (Natural [1 9]), keeps valid boards
;; ASSUME: board has at least one blank position
(check-expect (next-boards (cons 1 (rest BD1))) (list (cons 1 (cons 2 (rest (rest BD1))))
...
;; Board -> Pos
;; Given a board returns the first position that contains a blank
...
;; Return a list of 9 boards where the given position contains each integer between 1 and 9
(check-expect (fill-with-1-9 0 BD1) (list (cons 1 (rest BD1))
                                          (cons 2 (rest BD1))
                                          (cons 3 (rest BD1))
                                          (cons 4 (rest BD1))
                                          (cons 5 (rest BD1))
...
(check-expect (fill-with-1-9 1 (cons 1 (rest BD1))) (list (cons 1 (cons 1 (rest (rest BD1))))
...
(define (fill-with-1-9 p bd)
  (local [(define (build-one n) (fill-square bd p (+ n 1)))]
    (build-list 9 build-one)))

;; (listOf Board) -> (listOf Board)
;; Removes boards that are invalid
(check-expect (keep-valid (list (cons 1 (cons 1 (rest (rest BD1))))))
              empty)
...
;; Board -> Boolean
;; Consumes a board and returns true if no unit on the board has the same value twice
...
(check-expect (valid-board? (cons 4 (rest BD5))) false)

(define (valid-board? bd)
  (andmap no-duplicates?
          (map (lambda (unit) (map (lambda (p) (read-square bd p)) unit)) UNITS)))

;; Unit -> Boolean
;; Produces true if there is a duplicate value in the unit
(check-expect (no-duplicates? (list 1 2 3 4 5 6 7 8 9)) true)
(check-expect (no-duplicates? (list 2 2 3 4 5 6 7 8 9)) false)
(check-expect (no-duplicates? (list 1 2 3 4 5 6 7 8 1)) false)

(define (no-duplicates? unit)
  (if (empty? unit)
      true
      (if (and (not (false? (first unit))) (member? (first unit) (rest unit)))
          false
          (no-duplicates? (rest unit)))))
